...
**Claims need not be verbatim.** The linter accepts the cited section if at least 30% of the claim's load-bearing words (length ≥ 4, non-stopword) appear in the section, either literally or as a 4-character prefix in either direction (so `generates` matches `generate`, `routes` matches `route` / `routing`). Verbatim and whitespace-normalized substring hits still score highest (1.0 and 0.85); the looser tier scores `matched / total` and is accepted at `≥ 0.3`. Weak-but-passing matches (confidence below 0.7) are flagged as warnings so reviewers can spot paraphrases that wandered too far.

**Very low overlap → `claude` makes the call.** When token overlap drops **below 0.3** (paraphrase that strayed too far, or a hallucinated claim), the resolver shells out to the local `claude` CLI for a faithfulness verdict. Faithful paraphrase → clean pass. Invented or contradictory claim → hard `claim_unfaithful` error that re-spawns the agent with feedback. If `claude` is unreachable (not on PATH, timeout, etc.), the linter is permissive: the citation passes with a `claim_low_confidence` warning rather than blocking the run on tooling flakiness. The check uses the local CLI's own auth — no `ANTHROPIC_API_KEY` plumbing, no SDK dependency, no cache.
